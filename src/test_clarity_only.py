...
def scrape_single_county(county, url):
    """Scrape one Clarity county in its own thread with its own driver."""
    start = time.time()
    try:
        scraper = ClarityScraper(url, reuse_driver=None, save_files=False)
        result = scraper.scrape()
        duration = time.time() - start
        return county, result, duration, None
    except Exception as e:
        duration = time.time() - start
        return county, None, duration, str(e)
...
with ThreadPoolExecutor(max_workers=2) as executor:
    futures = {
        executor.submit(scrape_single_county, county, url): county
        for county, url in CLARITY_SITES.items()
    }
    for future in as_completed(futures):
        county, result, county_duration, error = future.result()

        print(f"\n[{len(results)+1}/4] Scraping {county}...")
        print("-"*70)

        if error:
            print(f"❌ FAILED after {county_duration:.1f}s - {error}")
            results.append({
                'county': county,
                'success': False,
