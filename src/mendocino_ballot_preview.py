#!/usr/bin/env python3
"""
Mendocino County ballot preview for the June 2, 2026 Primary Election.

Data source: PDF candidate list from mendocinocounty.gov (parsed manually on first run).
All candidate/measure data is stored in:
  election_data/local_races - Sheet1.csv       (contested races + measures)
  election_data/uncontested_races - Sheet1.csv  (uncontested races)

Run this script to regenerate the HTML from the CSVs.
"""
...
OUTPUT_FILE = _ROOT / "data" / "mendocino_ballot_preview_2026-06-02.html"

COUNTY = "Mendocino"
ELECTION_TITLE = "Mendocino County — June 2, 2026 Primary"
...
def main():
    ...
    # Strip leading ** from uncontested race names (Mendocino notation for no-incumbent)
    for u in uncontested:
        u["race"] = u["race"].lstrip("*").strip()
